...
public class SubscriptionFromPositionTest {
  ...
  public void setUp() {
    ...
    mConfig = new SubscriptionConfig(SubscriptionMode.SIMPLE, mListener)
  ...
  public void failedToSubscribe() throws InvalidJsonException {
    ...
    PduRaw responsePdu = mSerializer.parsePdu(
        "{'action':'rtm/subscribe/error','body':{'channel':'channel','error':'error_code','error_text':'error_message'}}");

    reply.setException(new PduException("error_outcome", responsePdu));
  ...
  public void subscribeUnsubscribe() {
    ...
    when(mConnection
        .send(eq("rtm/subscribe"), Matchers.any(), eq(SubscribeReply.class)))
    ...
    when(mConnection
        .send(eq("rtm/unsubscribe"), Matchers.any(), eq(UnsubscribeReply.class)))
  ...
  public void closeConnectionWhenGetUnsubscribeError() throws InvalidJsonException {
    ...
    PduRaw reason = mSerializer.parsePdu("{'action':'rtm/unsubscribe/error'," +
        "'body':{'subscription_id':'channel','error':'error_code','reason':'error_message'}}");
